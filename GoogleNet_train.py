...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'GoogleNet.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullGoogleNet.pth')  # 保存整个模型
    ...
    model = GoogLeNet(num_classes=num_classes, aux_logits=True, init_weights=True)
    model.to(device)
