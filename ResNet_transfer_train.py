...
def train_model(epoch, model, device, save_weights_path, save_visualization_result, model_name, train_loader,
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_loader, start=0):
            ...
            if step % 2 == 0:
                print("[epoch:%d %2d] loss:%.3f" % (epoch+1, step, running_loss_copy/2)) #每20组数据打印一次损失函数
...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'ResNet34_transfer.pth')
    ...
    model = resnet34()
    # model_weight_path = args['pre_weights']
    ...
    # 可视化结果的保存名称 默认为模型名
    model_name = "ResNet34_transfer"

    # 迭代轮数
    epochs = args['epochs']
    #训练模型
    # train_model(epochs, model, device, save_weights_path, save_visualize_dir, model_name, train_loader, val_loader,
    #             val_dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pre_weights', default='F:/实验结果/分类网络/旧的权重/resnet34.pth', help='使用迁移学习时的预训练权重')
