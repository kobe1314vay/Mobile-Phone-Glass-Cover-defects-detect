...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'MDNet.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullMDNet.pth')  # 保存整个模型
    ...
    model = MDNet(num_classes=num_classes)
    print(model)
    model.to(device)
