...
from model.mobilenetv2 import MobileNetV2, SEMobileNetV2
import os
import time
...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'mobilenetSEv2.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullmobilenetSEv2.pth')  # 保存整个模型
    ...
    modelv2 = MobileNetV2(num_classes=num_classes)
    model_weight_path = "./pre_weights/mobilenet_v2.pth"
    assert os.path.exists(model_weight_path), "file {} dose not exist.".format(model_weight_path)
    pre_weights = torch.load(model_weight_path, map_location=device)
    pretrained_dict = {k: v for k, v in pre_weights.items() if modelv2.state_dict()[k].numel() == v.numel()}

    modelSEv2 = SEMobileNetV2(num_classes=num_classes)
    # model_dict = modelSEv2.state_dict()
    # pretrained = {}
    ...
    # missing_keys, unexpected_keys = modelSEv2.load_state_dict(model_dict, strict=False)

    # freeze features weights
    # for param in modelSEv2.features.parameters():
    #     param.requires_grad = False

    modelSEv2.to(device)

    # 可视化结果的保存名称 默认为模型名
    model_name = "mobilenetSEv2"
    ...
    #训练模型
    t1 = time.time()
    # train_model(epochs, modelSEv2, device, save_weights_path, save_full_model_path, save_visualize_dir, model_name, train_loader, val_loader,
    #             val_dataset, print_train_loss)
    t2 = time.time()
    print('训练时长：{}'.format(t2-t1))
    #打印混淆矩阵 验证模型 保存实验结果
    modelSEv2.load_state_dict(torch.load(save_weights_path))
    modelSEv2.to(device)
    create_confuison_matrix(modelSEv2, val_loader, save_visualize_dir, model_name, device)
