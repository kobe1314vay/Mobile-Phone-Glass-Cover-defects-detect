...
def select_datasets(args):
    ...
    datasets_type = 'MPGC_Single_DET' -- 自定义数据集
                  ...
                  = 'Mini_ImageNet' -- Mini-ImageNet数据集
    MiniImageNet_json_root:Mini-ImageNet数据集的json文件根目录
    ...
    if datasets_type == 'MPGC_Single_DET':
        ...
        val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True,
                                 num_workers=num_workers)
    elif datasets_type == 'Mini_ImageNet':
        json_path = os.path.join(datasets_root, 'classes_name.json')

        train_dataset = Mini_ImageNet('F:/datasets/MiniImagenet', csv_name="new_train.csv", json_path=json_path,
                                           transform=data_transform["train"])

        val_dataset = Mini_ImageNet('F:/datasets/MiniImagenet', csv_name="new_val.csv", json_path=json_path,
...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'MDNet_transfer.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullMDNet_transfer.pth')  # 保存整个模型
    ...
    pre_model = resnet34()  # 创建原模型
    pre_model.load_state_dict(torch.load('./pre_weights/resnet34.pth'))  # 加载预训练模型
    pretrained_dict = pre_model.state_dict()  # 加载原模型的参数

    MDNet2_model = MDNet2()  # 创建新模型
    MDNet_model = MDNet()
    model_dict = MDNet2_model.state_dict()
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    MDNet2_model.load_state_dict(model_dict)

    # 在此加入深度可分离卷积
    MDNet2_model.layer1 = MDNet_model.layer1
    MDNet2_model.layer2 = MDNet_model.layer2
    MDNet2_model.layer3 = MDNet_model.layer3
    MDNet2_model.layer4 = MDNet_model.layer4

    # 冻结部分特征提取层权重 训练全连接层
    # for param in new_model.parameters():
    #     param.requires_grad = False
    # for p in MDNet2_model.layer1.parameters():
    #     p.requires_grad = False
    #
    # for p in MDNet2_model.layer2.parameters():
    #     p.requires_grad = False
    #
    # for p in MDNet2_model.layer3.parameters():
    #     p.requires_grad = False
    #
    # for p in MDNet2_model.layer4.parameters():
    #     p.requires_grad = False

    # change fc layer structure
    inchannel = MDNet2_model.fc.in_features
    MDNet2_model.fc = nn.Linear(inchannel, num_classes)
    MDNet2_model.to(device)

    # 可视化结果的保存名称 默认为模型名
    model_name = "MDNet_transfer"
    ...
    #训练模型
    train_model(epochs, MDNet2_model, device, save_weights_path, save_full_model_path, save_visualize_dir, model_name, train_loader, val_loader,
                val_dataset, print_train_loss)

    #打印混淆矩阵 验证模型 保存实验结果
    MDNet2_model.load_state_dict(torch.load(save_weights_path))
    MDNet2_model.to(device)
    create_confuison_matrix(MDNet2_model, val_loader, save_visualize_dir, model_name, device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets_type', default='MPGC_Single_DET', help='选择哪个数据集训练:\n'
                                                                                ...
                                                                                'Mini_ImageNet -- mini版的ImageNet\n')
    parser.add_argument('--datasets_root', default='F:/datasets/MPGCCLASS', help='数据集的根目录\n'
                                                                                 'Mini_MPGC_Single_DET and '
                                                                                 'MPGC_Single_DET -- F:/datasets/MPGCCLASS\n'
                                                                                 'Mini_ImageNet -- F:/datasets/MiniImagenet')
    ...
    parser.add_argument('--batch_size', default=4, type=int, help='Batch大小')
    parser.add_argument('--num_workers', default=4, type=int, help='cpu线程数')
    parser.add_argument('--epochs', default=30, type=int, help='迭代轮数')
    parser.add_argument('--pre_weights', default='./pre_weights/resnet34.pth', help='使用迁移学习时的预训练权重')
