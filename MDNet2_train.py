...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'MDNet2.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullMDNet2.pth')  # 保存整个模型
    ...
    model = MDNet2(num_classes=num_classes)
    print(model)
    model.to(device)
