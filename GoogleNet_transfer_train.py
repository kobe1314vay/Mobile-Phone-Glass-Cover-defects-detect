...
def main(args):
    ...
    save_weights_path = os.path.join(save_weights_dir, 'GoogleNet_transfer.pth')
    save_full_model_path = os.path.join(save_weights_dir, 'FullGoogleNet_transfer.pth')  # 保存整个模型
    ...
    model_weight_path = './pre_weights/googlenet_Imagenet.pth'
    ...
    # 可视化结果的保存名称 默认为模型名
    model_name = "GoogleNet_transfer"

    #是否每10个epoch打印训练集损失函数
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets_type', default='Mini_MPGC_Single_DET', help='选择哪个数据集训练:\n'
    ...
    parser.add_argument('--print_train_loss', default=False, help='是否每10个epoch打印训练集损失函数')
